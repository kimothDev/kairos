...
export async function getRecommendations(
  ...
  const focusContextKey = createContextKeyFromParts(taskType, energyLevel, false);
  const breakContextKey = createContextKeyFromParts(taskType, energyLevel, true);
  ...
  const focusParams = modelState[focusContextKey]?.[recommendation.focusDuration] || { alpha: 1.5, beta: 1 };
  ...
  if (fadeFactor > 0 && taskType) {
    ...
    if (shortSessionTasks.includes(normalizedTask)) {
      recommendation.focusDuration = roundToNearest5(recommendation.focusDuration * 0.8);
    }
  }

  // Clamp and round the final focus duration
  recommendation.focusDuration = Math.min(60, Math.max(5, roundToNearest5(recommendation.focusDuration)));

  // Normalize break time
  recommendation.breakDuration = recommendation.breakDuration <= 1
    ? 5
    : Math.min(20, Math.max(5, roundToNearest5(recommendation.breakDuration)));
